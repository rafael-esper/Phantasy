...
import static core.VergeEngine.die;
...
public class PSDungeon {
	...
	public enum DungeonType {
		FIRE	("/ps/images/original/dungeon/Fire/"),
		GREY	("/ps/images/original/dungeon/Grey/"),
		GREEN	("/ps/images/original/dungeon/Green/"),
		YELLOW	("/ps/images/original/dungeon/Yellow/"),
		BLUE	("/ps/images/original/dungeon/Blue/"),
		ILLUSION("/ps/images/original/dungeon/Illusion/"),
		COLOR	("/ps/images/original/dungeon/Color/"),
		MOTA	("/ps/images/original/dungeon/Mota/"),
		ORANGE	("/ps/images/original/dungeon/Orange/"),
		EMERALD	("/ps/images/original/dungeon/Emerald/"),
		DARK	("/ps/images/original/dungeon/Dark/"),
		RUBY	("/ps/images/original/dungeon/Ruby/"),
		COLD	("/ps/images/original/dungeon/Cold/"),
		;
	...
	public void startDungeon() {
		...
		System.out.println("Log: Dungeon init, timer: " + timer + "\t" + GUI.cycleTime);
		initDungeonImages(curDungeon.getType().path);
		System.out.println("Log: Dungeon end, timer: " + timer + "\t" + GUI.cycleTime);
		...
		if(getAlreadyInside()) {
			entity.get(player).setface(PSGame.getDungeonFace());
			isDark = false;
		}
		else {
		...
		while(!die) {
			...
			if(isDark && (up || left || right || down)) {
				int zone = getfrontzone(entity.get(player), -1);
	...
	private void turnRoutine(Entity e, boolean counter) {
		...
		e.setface(nextDirection(e.face, counter));
	...
	public static int gettile(int x, int y) {
		return current_map.gettile(x/16, y/16, 0);
	}
	...
	private void walkTo(Entity e, int xpos, int ypos, boolean walkbackwards) {
		if(current_map.gettile(xpos/16, ypos/16, 0) != WALL) {
			
			// Can't traverse stairs or doors when walking backwards
			if(walkbackwards && current_map.gettile(xpos/16, ypos/16, 0) != FLOOR)
				return;
			// Can't traverse an locked door
			if(current_map.gettile(xpos/16, ypos/16, 0) > 4 && current_map.gettile(xpos/16, ypos/16, 0) < 8)
				return;			
			
			e.setxy(xpos,  ypos);
			
			// If it's over an open door, advance one tile
			if(current_map.gettile(e.getx()/16, e.gety()/16, 0) == OPEN_DOOR || 
				current_map.gettile(e.getx()/16, e.gety()/16, 0) == OPEN_MAGIC_DOOR ) {
				if(showDungeon) {
				...
				switch(current_map.gettile(e.getx()/16, e.gety()/16, 0)) {
					case STAIRS_UP:
					case STAIRS_DOWN:
					case ROOM:
						callZone(e, 0);
	...
	private void open(int xpos, int ypos) {
		int tile = current_map.gettile(xpos/16, ypos/16, 0);
		switch(tile) {
			case DOOR:
			case LOCKED_DOOR:
