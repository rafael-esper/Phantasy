...
import static ps.oo.menuGUI.MenuStack.getMaxTextLength;
...
import ps.oo.menuGUI.MenuScrollerText;
import ps.oo.menuGUI.MenuStack;
import ps.oo.menuGUI.MenuTextBox;
import domain.CHR;
...
public class PSMenu {
	...
	public static void initPSMenu(ScreenSize screenSize) {
		...
		instance.moreIcon = new VImage(load("/ps/oo/menuGUI/Next_Icon.png"));
	...
	public static void menuOn() {
		hookbutton(4, "ps.oo.PSMenuMain.menu");
		if(PSGame.gameData != null && PSGame.gameData.enableCheats) {
			hookkey(Script.SCAN_M, "ps.oo.PSMenuMain.cheatMenu");
		}
	}
	...
	public static void startScene(Scene scene, LargeEntity largeEntity) {
		if(!Script.TEST_SIMULATION) {
			...
			instance.entitySprite.image = PSGame.getCHR(PS1CHR.IMG_ENTITIES_LARGE).frames[largeEntity.ordinal()];
	...
	public static void startScene(Scene scene, CHR chr) {
		if(!Script.TEST_SIMULATION) {
			instance.entitySprite = new VImage(chr.fxsize, chr.fysize);
			instance.entitySprite.image = chr.frames[0];
	...
	private static void startScene(Scene scene, EntityType entityType, int numIndex) {
		...
		Color transC = new Color(PSGame.getCHR(PS1CHR.IMG_ENTITIES).frames[0].getRGB(0, 0));	
		instance.entitySprite.tgrabRegion(0, 0, 35, isHalf ? 51 : 90, 0, 0, transC, PSGame.getCHR(PS1CHR.IMG_ENTITIES).frames[(entityType.ordinal())*4 + numIndex]);
		instance.entityY = 183 - instance.entitySprite.height;
		...
		for(int j=0; j<rows.size(); j++) {		
			...
			if(j+2 < rows.size()) {
				...
				if(!TEST_SIMULATION) {
					...
					log(Integer.toString(TEST_OPTIONS[TEST_POS]));
