...
public class BattlePosition {
	...
	public static int[] distributePositions(int wSize, int num, SceneType scene) {
		...
		System.out.println("size: " + wSize + "\t" + mSize + "\t" + scene.getScreenSize());
		
		int numFrontRow = Math.min(num, mSize.maxNum(scene));
		
		if(num > numFrontRow) {
			System.out.println("Too much objects: " + num + ". Max permitted: " + (numFrontRow));
		}
		...
		for(int i=0; i<num; i++) {
			...
			System.out.println("\t" + i + " on position " + ret[i]);
		}
	...
	public static void main(String args[]) {
		...
		/*System.out.println(Size.TINY.maxSize(320));	// 5 + 4	= 9
		System.out.println(Size.SMALL.maxSize(320));	// 4 + 3	= 7
		System.out.println(Size.MEDIUM.maxSize(320));	// 3 + 2	= 5
		System.out.println(Size.BIG.maxSize(320));		// 2 + 1	= 3
		System.out.println(Size.HUGE.maxSize(320));		// 1 + 0	= 1*/
