...
import ps.oo.PSEffect.Effect;
import ps.oo.PSEffect.EffectPlace;
import ps.oo.PSGame.Trap;
import ps.oo.PSLibCHR.PS1CHR;
...
import ps.oo.PSLibSpell.PS1Spell;
...
public class PSGame {
	...
	public static void initGameScreen(ScreenSize screenSize) {
		...
		GameData.rb = ResourceBundle.getBundle("ps/lang/Script", gameData.locale);
...
//		if(gameType == PSGame.GameType.PS_ORIGINAL || gameType == PSGame.GameType.PS_BATTLE 
	...
	public static void playSound(PS1Sound sound) {
		if(sound == null || soundLIB == null || TEST_SIMULATION) {
			if(!TEST_SIMULATION)
				System.err.println("Sound or soundLIB is null.");
			return;
		}
...
//		} FM Not yet else {
	...
	public static Item getItem(PSLibItem.OriginalItem index) {
		if(itemLIB == null) {
			System.err.println("Item LIB not initialized.");
			return null;
		}
	...
	public static Enemy getEnemy(GenericEnemy index) {
		if(enemyLIB == null) {
			System.err.println("Enemy LIB not initialized.");
			return null;
		}
	...
	public static void transportOn() {
		canTransport = true;
		hookbutton(1, "ps.oo.PSGame.verifyTransport");
	}
	...
	private static void disembark() {
		getParty().disembark(entity.get(player).getx() /16, entity.get(player).gety() /16);
		gameData.onGroundVehicle = false;
	...
	public static boolean landrover() {
		...
		int x = 0, y = 0;
		if(player != -1) {
			x = entity.get(player).getx() /16;
			y = entity.get(player).gety() /16;
		}
		else {
	...
	public static boolean hovercraft(boolean enter) {
		...
		int x = 0, y = 0;
		if(player != -1) {
			Entity e = entity.get(player);
			switch(entity.get(player).face) {
	...
	public static boolean isWater(int x, int y) {
		int tile = current_map.gettile(x,  y,  0);
		if(tile == 7 || tile == 230 || tile == 250 ||
				(tile>=180 && tile<=189) ||
				(tile>=200 && tile<=209) ||
				(tile>=260 && tile<=269) ||
				(tile>=280 && tile<=289) ||
	...
	public static void spaceportTransition(int direction, int whenStop, City destiny, int gotox, int gotoy) {
		...
		while(true) {
			...
			if(direction == Entity.WEST && entity.get(player).getx()/16 < whenStop) break;
			if(direction == Entity.EAST && entity.get(player).getx()/16 > whenStop) break;
			if(direction == Entity.NORTH && entity.get(player).gety()/16 < whenStop) break;
			if(direction == Entity.SOUTH && entity.get(player).gety()/16 > whenStop) break;
			if(b1) break;
	...
	public static void EntStart() {
		...
		if(event_entity >= 0 && event_entity < entity.size()) {
			tmpEntSpeed = entity.get(event_entity).speed;
	...
	public static void EntFinish() {
		PSMenu.menuOn();
		if(event_entity >= 0 && event_entity < entity.size()) {
			entity.get(event_entity).speed = tmpEntSpeed;
		}
		unpauseplayerinput();
	}	
	...
	public static void warp(int i, int j, boolean rendermap) {
		...
		entity.get(player).setxy(i*16, j*16);
	}
	...
	public static String getString(String s) {
		try {
			...
			System.err.println("String " + s + " not found.");
			return s; 
		}
	}
	...
	public static void regroup(int xAdjust, int yAdjust) {
		if(TEST_SIMULATION || entity == null || player==-1 || entity.get(player) == null) {
			return;
		}
		int x = (entity.get(player).getx()+16*xAdjust)/16;
		int y = (entity.get(player).gety()+16*yAdjust)/16;
		entity.get(player).setxy(x*16, y*16);
		entity.get(player).face=Entity.SOUTH;
	}
	...
	public static boolean chest(int mesetas, Trapped trapped, Item item) {
		...
		if(membersWhoKnowTrap.size() > 0) {
			...
			if(openChest == 2) {
				...
				int counter = 0;
				while(chosenMember.getMp() < PS1Spell.TRAP.getMpCost() && membersWhoKnowTrap.size() > counter) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			if(loaded != null) {
				...
				if(getCurrentDungeon() != Dungeon.NONE) {
					...
					System.out.println("RBP\t" + gameData.onGroundVehicle);
	...
	public static void saveGame() {
		...
		gameData.gotox = entity.get(player).getx()/16;
		gameData.gotoy = entity.get(player).gety()/16;

		if(getCurrentDungeon() != Dungeon.NONE) {
			if(!currentDungeon.getAlreadyInside()) {
				return;
			}
			gameData.dungeonFace = entity.get(player).face;
		}
	...
	public static VImage getImage(PS1Image image) {
		if(image == null || imageLIB == null) {
			System.err.println("Image or imageLIB is null.");
			return null;
		}
	...
	public static CHR getCHR(PS1CHR anim) {
		if(anim == null || animLIB == null) {
			System.err.println("Anim or animLIB is null.");
			return null;
		}
		if(!animLIB.containsKey(anim)) {
			animLIB.put(anim, new CHR(load(anim.getUrl())));
		}
	...
	public static void getOutOfCurrentZone() {
		...
		int curx = entity.get(player).getx()/16;
		int cury = entity.get(player).gety()/16;
		...
		entity.get(player).setxy(curx*16, cury*16);
	}	
	...
	public static void languageMenu(int posx, int posy) {
		...
		case 0: 
			...
			GameData.rb = ResourceBundle.getBundle("ps/lang/Script", PSGame.gameData.locale);
			break;
		case 1: 
