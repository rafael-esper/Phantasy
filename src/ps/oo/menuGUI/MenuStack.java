		...
		if(back == null && backAnim == null) {
			...
			if(showPlayers) { // Show .chr Players
				...
				for(int i=0;i<numPlayers;i++) {
					screen.blitentityframe(308 - (countParty(player)-1)*20 + (playerOrder[numPlayers-1][i]*40), downPos+64, e.chr, e.chr.idle[1]);
					e = e.getFollower();
				}
			}
