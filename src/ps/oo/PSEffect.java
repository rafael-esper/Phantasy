...
import ps.PSDungeon;
import ps.oo.Enemy.CanChat;
import ps.oo.Enemy.CanRope;
import ps.oo.Enemy.CanTalk;
...
public class PSEffect {
	...
	public EffectOutcome callEffect() {
		...
		switch (this.effect) {
			...
			case RUN: 
			case ESCAPE:
				if(runRoutine(targets, this.effect == Effect.RUN)) {
			...
			case CHAT:
			case TELE:
				if(talk(targets, user, this.effect)) {
					return EffectOutcome.SUCCESS;
				} else {
					return EffectOutcome.NONE;
				}
	...
	private boolean runRoutine(List<Battler> battlers, boolean random) {
		...
		if(PSGame.getCurrentDungeon() != Dungeon.NONE && PSGame.currentDungeon.deadEnd()) {
			chanceToRun = 0;
			System.out.println("On dead end!");
		}

		int chance = (int) (random ? 1 + Script.random(0, 255) : 1);
		
		System.out.println("Got " + chance + " of " + chanceToRun);
