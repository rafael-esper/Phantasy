...
import static ps.oo.PSGame.playSound;
...
import ps.oo.Enemy.CanProt;
import ps.oo.Enemy.FireRes;
import ps.oo.Enemy.HasItem;
import ps.oo.Enemy.Special;
import ps.oo.Item.EquipPlace;
import ps.oo.Item.ItemType;
import ps.oo.PSEffect.Effect;
import ps.oo.PSEffect.EffectOutcome;
import ps.oo.PSEffect.EffectPlace;
import ps.oo.PSEffect.EffectTarget;
...
import ps.oo.menuGUI.MenuType.State;
import core.Script;
import domain.CHR;
	...
	public BattleOutcome startBattle(List<Battler> battlers) {
		...
		for(Battler battler: battlers) {
			if(battler instanceof EnemyBattler) {
				numOfEnemies++;
				if(((EnemyBattler)battler).getEnemy().getChr().fxsize > maxSize) {
					maxSize = ((EnemyBattler)battler).getEnemy().getChr().fxsize;
				}
		...
		for(Battler battler: battlers) {
			if(battler instanceof EnemyBattler) {
				...
				MenuCHR enemySprite = new MenuCHR(battlePositions[pos] - (enemy.getChr().fxsize/2), ((EnemyBattler) battler).getVerticalPos(), enemy.getChr());
		...
		for(Battler battler: battlers) {
			...
			if(battler instanceof EnemyBattler) {
				...
				if(p.equipment[EquipPlace.WEAPON.ordinal()] != null) {
					...
					battler.sprite = new MenuCHR(0, 0, new CHR("battle/weapon_ps1/Claw.chr"));
				}
		...
		if(special == Special.CURE) {
			...
			for(Integer naturalIndex: Battler.getNaturalOrder(battlers)) {
				...
				if(bTarget instanceof PartyMember && bTarget.getHp() > 0) {
					...
					//System.out.println(b.getName() + " uses Thunder2 causing " + damage + " damage on " + bTarget.getName());
				}
			}
		}
