...
public class Party implements Serializable {
	...
	public void setOrder(int[] orders) {
		...
		if(sum!=0) {
			System.err.println("Party::Invalid Party order.");
			return;
		}
		this.order = orders;
	}
	...
	public void allocate(int gotox, int gotoy) {
		...
		setplayer(last);
		entity.get(player).speed = WALKING_SPEED;
	...
	public void reallocate() {
		...
		int x = entity.get(player).getx() /16;
		int y = entity.get(player).gety() /16;
...
/*		for(PartyMember p: members) {
	...
	public void embark(int x, int y, String strChar) {
		int direction = player == -1 ? Entity.NORTH : entity.get(player).face;
		deallocate();
		...
		entity.get(player).speed = TRANSPORT_SPEED;
		entity.get(player).face = direction;
	}
	
	public void disembark(int x, int y) {
		entity.get(player).speed = WALKING_SPEED;
		entity.get(player).active = false;
		entity.get(player).visible = false;		
